...
void MediaModule::render_items()
{
    for (int i = 0; i < media_vec.size(); ++i)
    {
        ...
        this_media.thumbnail_node->set_position(sf::Vector2f(12, 10 + (int)(unit_height * ui_scale * i)));
        ...
        sf::Vector2f thumbnail_base_scale = sf::Vector2f(1, 1).componentWiseDiv(sf::Vector2f(this_media.thumbnail_tex->get_size()));
        this_media.thumbnail_node->set_scale(thumbnail_base_scale * std::floor(unit_height * ui_scale - 4));
    }
}
