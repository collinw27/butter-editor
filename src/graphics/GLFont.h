#ifndef BUTTERGL_FONT_H
#define BUTTERGL_FONT_H
...
{
    FT_Face font_face;
    std::map<unsigned int, std::map<char, FontChar>> loaded_chars {};

public:

    GLFont(std::filesystem::path ttf_location);
    ~GLFont();
    void load_char_map(unsigned int char_size);
    std::map<char, FontChar>& get_char_map(unsigned int char_size);
};
